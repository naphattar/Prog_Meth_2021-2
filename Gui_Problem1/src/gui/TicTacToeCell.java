...
public class TicTacToeCell extends Pane{
		...
		if(!(GameLogic.getInstance().isGameEnd())){
			if(this.isDrawn == false) {
				if(GameLogic.getInstance().isOturn()) {
					this.draw(new Image(oURL),Color.AQUA);
				}else {
					this.draw(new Image(xURL), Color.YELLOW);
				}
				GameLogic.getInstance().drawNumber(this.getxPosition(), this.getyPosition());
			}
		}
	...
	public void initializeCellColor() {
		this.setDrawn(false);
		//To be implemented
		this.setBackground(new Background(new BackgroundFill(this.getBaseColor(),CornerRadii.EMPTY, Insets.EMPTY)));
		this.setDrawn(false);
	}
